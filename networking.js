...
export const socket = io("https://3dgame-api.khoirony.fun"); // Ganti URL jika perlu
...
// Fungsi internal di dalam networking
...
function showChatBubble(targetGroup, message) {
    ...
    if (message === "/clear") {
        targetGroup.userData.currentMessage = "";
        return;
    }
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        if (metrics.width > maxWidth && i > 0) { lines.push(currentLine.trim()); currentLine = words[i] + ' '; } 
    ...
    targetGroup.userData.currentMessage = message; 
}