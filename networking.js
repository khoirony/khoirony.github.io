...
// === 1. BUAT / AMBIL "KTP" PLAYER (PERSISTENT ID) ===
let myPlayerId = localStorage.getItem('mmo_player_id');
if (!myPlayerId) {
    // Jika belum punya ID (baru pertama kali main), buat ID acak
    myPlayerId = 'player_' + Math.random().toString(36).substr(2, 9);
    localStorage.setItem('mmo_player_id', myPlayerId);
}

// === 2. KIRIM "KTP" KE SERVER SAAT KONEK ===
export const socket = io("https://3dgame-api.khoirony.fun", {
...
function createOtherPlayerModel() {
    ...
    group.userData = { armL: aL, armR: aR, legL: lL, legR: lR, moveTime: 0, lastPos: new THREE.Vector3(), currentMessage: "" };
    return group;
}
...
function showChatBubble(targetGroup, message) {
    ...
    ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'; ctx.roundRect(10, 10, canvas.width - 20, canvas.height - 20, 20); ctx.fill(); ctx.strokeStyle = 'white'; ctx.lineWidth = 4; ctx.stroke();
    ctx.fillStyle = 'white'; ctx.font = `bold ${fontSize}px Arial`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ...
    const scaleFactor = 0.006; sprite.scale.set(canvas.width * scaleFactor, canvas.height * scaleFactor, 1); sprite.position.y = 2.8 + ((canvas.height - 128) * scaleFactor / 2); 
