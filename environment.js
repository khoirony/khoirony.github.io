...
export const interactiveHouses = []; export const beds = []; 
...
export function createWorld(scene) {
    ...
    createTent(scene, -35, -5, 0); createLake(scene, 35, 10, 15); createTent(scene, 45, 20, -Math.PI/4);
    createMountain(scene, 0, -80); createMountain(scene, 80, -40); 
    createHill(scene, 15, -45, 1.5); createHill(scene, -30, 20, 2);
}

// === FACTORY OBJEK DINAMIS (DIPANGGIL OLEH GAME.JS) ===
...
export function createPigMesh() {
    ...
    const legFL = createLeg(0.25, 0.4); const legFR = createLeg(-0.25, 0.4); const legBL = createLeg(0.25, -0.4); const legBR = createLeg(-0.25, -0.4);
...
export function createTorchMesh() {
    ...
    const light = new THREE.PointLight(0xffaa00, 0, 15); light.position.y = 2.5; group.add(light);
    return { mesh: group, light: light, fire: fire };
}

export function createMadingMesh(title, lines) {
    const group = new THREE.Group(); const poleMat = new THREE.MeshStandardMaterial({ color: 0x3d2314 }); 
    ...
    const gradient = ctx.createLinearGradient(0, 0, 0, canvas.height); gradient.addColorStop(0, '#ffffff'); gradient.addColorStop(1, '#fef3c7'); ctx.fillStyle = gradient; ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.strokeStyle = '#b45309'; ctx.lineWidth = 15; ctx.strokeRect(0, 0, canvas.width, canvas.height); ctx.fillStyle = '#dc2626'; [[35,35], [989,35], [35,477], [989,477]].forEach(pos => { ctx.beginPath(); ctx.arc(pos[0], pos[1], 10, 0, Math.PI * 2); ctx.fill(); });
    ctx.fillStyle = '#991b1b'; ctx.font = '900 55px "Segoe UI", Arial, sans-serif'; ctx.textAlign = 'center'; ctx.fillText(title, canvas.width / 2, 85); ctx.beginPath(); ctx.moveTo(250, 110); ctx.lineTo(774, 110); ctx.lineWidth = 4; ctx.strokeStyle = '#d97706'; ctx.stroke();
    ctx.fillStyle = '#1e293b'; ctx.font = 'bold 34px "Segoe UI", Arial, sans-serif'; ctx.textAlign = 'left'; let startY = 180; lines.forEach(l => { ctx.fillText(l, 80, startY); startY += 60; });

    const texture = new THREE.CanvasTexture(canvas); texture.needsUpdate = true; texture.colorSpace = THREE.SRGBColorSpace; 
    const board = new THREE.Mesh(new THREE.BoxGeometry(4.4, 2.4, 0.1), [new THREE.MeshStandardMaterial({color:0xdeb887}), new THREE.MeshStandardMaterial({color:0xdeb887}), new THREE.MeshStandardMaterial({color:0xdeb887}), new THREE.MeshStandardMaterial({color:0xdeb887}), new THREE.MeshStandardMaterial({ map: texture }), new THREE.MeshStandardMaterial({color:0xdeb887})]);
    board.position.set(0, 2.2, 0.15); board.castShadow = true; group.add(board); 
    return group;
}

// (Fungsi House, Tent, Lake dll dibiarkan di bawah persis seperti kodenya sebelumnya)
...
function createLake(scene, x, z, radius) { const water = new THREE.Mesh(new THREE.CylinderGeometry(radius, radius, 0.5, 32), new THREE.MeshStandardMaterial({ color: 0x1E90FF, transparent: true, opacity: 0.8 })); water.position.set(x, -0.5, z); scene.add(water); solidGrounds.push(water); const sand = new THREE.Mesh(new THREE.TorusGeometry(radius + 1, 1, 8, 32), new THREE.MeshStandardMaterial({ color: 0xEEDC82 })); sand.position.set(x, -0.2, z); sand.rotation.x = Math.PI / 2; scene.add(sand); solidGrounds.push(sand); }