...
export const botNPC = new THREE.Group();
...
function createBotNPC(scene) {
    ...
    const pantMat = new THREE.MeshStandardMaterial({ color: 0x0000aa }); 
    const skinMat = new THREE.MeshStandardMaterial({ color: 0x999999, metalness: 0.6, roughness: 0.4 }); 
    
    function createLimb(w, h, d, mat, yPivot) { 
    ...
    const botBody = new THREE.Mesh(bodyGeo, shirtMat); 
    botBody.position.y = 1.125; 
    botBody.castShadow = true;

    const bArmL = createLimb(0.25, 0.75, 0.25, skinMat, 1.5); bArmL.position.x = 0.375;
    const bArmR = createLimb(0.25, 0.75, 0.25, skinMat, 1.5); bArmR.position.x = -0.375;
    const bLegL = createLimb(0.25, 0.75, 0.25, pantMat, 0.75); bLegL.position.x = 0.125;
    const bLegR = createLimb(0.25, 0.75, 0.25, pantMat, 0.75); bLegR.position.x = -0.125;
    
    // Tangan dilipat di dada
    bArmL.rotation.x = -1.0; bArmL.rotation.z = -0.8;
    bArmR.rotation.x = -1.0; bArmR.rotation.z = 0.8;
    
    const headGroup = new THREE.Group();
    headGroup.position.y = 1.5;
    
    const monitorGeo = new THREE.BoxGeometry(0.7, 0.6, 0.5);
    const monitorMat = new THREE.MeshStandardMaterial({ color: 0x222222, metalness: 0.8, roughness: 0.2 });
    ...
    screen.position.set(0, 0.3, 0.251); 
    
    const eyeGeo = new THREE.PlaneGeometry(0.1, 0.05);
    const faceFeatureMat = new THREE.MeshBasicMaterial({ color: 0x000000 });
    const eyeL = new THREE.Mesh(eyeGeo, faceFeatureMat); eyeL.position.set(-0.12, 0.35, 0.252);
    const eyeR = new THREE.Mesh(eyeGeo, faceFeatureMat); eyeR.position.set(0.12, 0.35, 0.252);
    const mouthGeo = new THREE.PlaneGeometry(0.2, 0.03);
    const mouth = new THREE.Mesh(mouthGeo, faceFeatureMat); mouth.position.set(0, 0.22, 0.252);
    
    const antenaGeo = new THREE.CylinderGeometry(0.02, 0.02, 0.3);
    const antenaMat = new THREE.MeshStandardMaterial({ color: 0xaaaaaa });
    const antena = new THREE.Mesh(antenaGeo, antenaMat); antena.position.set(0, 0.75, 0);
    
    const bulbGeo = new THREE.SphereGeometry(0.06);
    const bulbMat = new THREE.MeshBasicMaterial({ color: 0xff0000 }); 
    const bulb = new THREE.Mesh(bulbGeo, bulbMat); bulb.position.set(0, 0.9, 0);
    
    headGroup.add(monitor, screen, eyeL, eyeR, mouth, antena, bulb);
    ...
    qCtx.fillStyle = '#ffff00'; 
    qCtx.shadowColor = "rgba(0,0,0,0.8)"; 
    qCtx.shadowOffsetX = 3; qCtx.shadowOffsetY = 3; qCtx.shadowBlur = 5;
    qCtx.fillText('❓', 64, 64);
    ...
    questionMark.position.set(0, 2.8, 0); 
    botNPC.userData.questionMark = questionMark; 
    
    botNPC.add(botBody, bArmL, bArmR, bLegL, bLegR, headGroup, questionMark);
    
    // Posisi yang aman, tidak menabrak obor
    botNPC.position.set(0, 0, 3); 
    botNPC.rotation.y = 0; 
    scene.add(botNPC);
    
    botBody.updateMatrixWorld(true); 
    wallObjects.push(new THREE.Box3().setFromObject(botBody));
}
...
export function createBrunoStylePortfolio(scene) {
    ...
    fontLoader.load('https://unpkg.com/three@0.160.0/examples/fonts/helvetiker_bold.typeface.json', function (font) {
        ...
        const subMesh = new THREE.Mesh(subGeo, new THREE.MeshStandardMaterial({ color: 0xffd700 }));
    ...
    const createPedestal = (x, z, colorHex, labelText, geometryShape) => {
        ...
        const icon = new THREE.Mesh(geometryShape, new THREE.MeshStandardMaterial({ color: colorHex, flatShading: true })); 
        icon.position.y = 2.0; icon.castShadow = true; group.add(icon);
...
export function createTreeMesh() { 
    const group = new THREE.Group(); 
    const trunk = new THREE.Mesh(treeTrunkGeo, treeWoodMat); 
    trunk.position.y = 1.5; trunk.castShadow = true; group.add(trunk); 
    
    const leaves = new THREE.Mesh(treeLeavesGeo, treeLeafMat); 
...
export function createPigMesh() { 
    ...
    const head = new THREE.Mesh(pigHeadGeo, pigMat); head.position.set(0, 1, 0.6); head.castShadow = true; 
    pig.add(body, head); 
    
    function createLeg(lx, lz) { 
        const legGroup = new THREE.Group(); 
        const leg = new THREE.Mesh(pigLegGeo, pigMat); leg.position.y = -0.2; leg.castShadow = true; 
        legGroup.add(leg); legGroup.position.set(lx, 0.4, lz); 
        return legGroup; 
    } 
...
export function createMadingMesh(title, lines) { 
    ...
    ctx.font = '900 110px "Palatino Linotype", "Book Antiqua", Palatino, serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; 
    ...
    ctx.fillStyle = '#000000'; ctx.font = 'bold 64px "Segoe UI", Arial, sans-serif'; ctx.textAlign = 'left'; 
    let startY = 350; 
    ...
    const canvasMaterial = new THREE.MeshStandardMaterial({ map: texture, roughness: 1.0, emissive: new THREE.Color(0x444444), emissiveMap: texture, emissiveIntensity: 0.8 }); 
    ...
    const spotLight = new THREE.SpotLight(0xfffabb, 60); spotLight.position.set(0, 7.0, 4.0); spotLight.target = board; spotLight.angle = Math.PI / 2.5; spotLight.penumbra = 0.8; spotLight.decay = 2; spotLight.distance = 20; spotLight.castShadow = true; group.add(spotLight); 
...
function createShop(scene, x, z, rotY = 0) { 
    const group = new THREE.Group(); 
    const table = new THREE.Mesh(new THREE.BoxGeometry(4, 1.2, 2), new THREE.MeshStandardMaterial({color: 0x8b4513})); table.position.y = 0.6; table.castShadow = true; group.add(table); 
    const roof = new THREE.Mesh(new THREE.BoxGeometry(5, 0.2, 3), new THREE.MeshStandardMaterial({color: 0x1e90ff})); roof.position.set(0, 3.5, 0); roof.rotation.x = -Math.PI/16; roof.castShadow = true; group.add(roof); 
    ...
    const sign = new THREE.Mesh(new THREE.BoxGeometry(3.5, 1, 0.1), [signMat, signMat, signMat, signMat, signTexMat, signMat]); sign.position.set(0, 3.5, 1.55); group.add(sign); 
    
    const light = new THREE.PointLight(0xffffff, 100, 25); light.position.set(0, 2.5, 0); group.add(light); 
    
    group.position.set(x, 0, z); group.rotation.y = rotY; scene.add(group); shops.push({ position: new THREE.Vector3(x, 0, z) }); 
    table.updateMatrixWorld(true); addWall(table); 
}

function createHollowHouse(scene, x, z, rotY = 0) { 
    const group = new THREE.Group(); const wallMat = new THREE.MeshStandardMaterial({ color: 0xfff8dc }); const width = 8, depth = 8, height = 4, thickness = 0.5; 
    const floor = new THREE.Mesh(new THREE.BoxGeometry(width, 0.2, depth), new THREE.MeshStandardMaterial({ color: 0x654321 })); floor.position.set(0, 0.1, 0); floor.receiveShadow = true; group.add(floor); 
    ...
    const roof = new THREE.Mesh(new THREE.ConeGeometry(7, 3, 4), new THREE.MeshStandardMaterial({ color: 0x8b0000 })); roof.position.set(0, height + 1.5, 0); roof.rotation.y = Math.PI / 4; roof.castShadow = true; group.add(roof); 
    
    const bedMat = new THREE.MeshStandardMaterial({ color: 0x1e3a8a }); const bed = new THREE.Mesh(new THREE.BoxGeometry(2.5, 0.6, 4), bedMat); bed.position.set(-2, 0.4, -1.5); bed.castShadow = true; group.add(bed); 
...
function createTent(scene, x, z, rotY = 0) { 
    ...
    tent.position.set(x, 1.5, z); tent.rotation.y = rotY + Math.PI / 4; tent.castShadow = true; tent.receiveShadow = true; scene.add(tent); addWall(tent); solidGrounds.push(tent); 
}
...
function createHill(scene, x, z, scale) { 
    ...
    hill.position.set(x, 0, z); hill.scale.set(scale, scale * 0.4, scale); hill.receiveShadow = true; scene.add(hill); solidGrounds.push(hill); 
}
...
function createMountain(scene, x, z) { 
    ...
    mtn.position.set(x, 25, z); mtn.castShadow = true; mtn.receiveShadow = true; scene.add(mtn); solidGrounds.push(mtn); 
}
