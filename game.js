...
import { createWorld, solidGrounds, wallObjects, interactiveHouses, beds, createPigMesh, createTreeMesh, createTorchMesh, createMadingMesh } from './environment.js';
...
const armL = createLimb(0.25, 0.75, 0.25, 0xffccaa, 1.5); armL.position.x = 0.375; player.add(armL); const armR = createLimb(0.25, 0.75, 0.25, 0xffccaa, 1.5); armR.position.x = -0.375; player.add(armR);
const legL = createLimb(0.25, 0.75, 0.25, 0x0000aa, 0.75); legL.position.x = 0.125; player.add(legL); const legR = createLimb(0.25, 0.75, 0.25, 0x0000aa, 0.75); legR.position.x = -0.125; player.add(legR);
...
const jumpBtn = document.getElementById('jump-btn'); const triggerJump = (e) => { e.preventDefault(); if (!isJumping && !isSleeping) { velocityY = 0.3; isJumping = true; } }; jumpBtn.addEventListener('touchstart', triggerJump, { passive: false }); jumpBtn.addEventListener('mousedown', triggerJump);
...
socket.on('updateNPCs', (serverPigs) => {
    for (let id in serverPigs) {
        ...
        if (!localPigs[id]) {
            let pigObj = createPigMesh(); scene.add(pigObj.mesh);
        ...
        localPigs[id].targetX = sPig.x; localPigs[id].targetZ = sPig.z; localPigs[id].targetRotY = sPig.rotationY; localPigs[id].state = sPig.state; localPigs[id].riddenBy = sPig.riddenBy;
    }
});
...
actionBtn.addEventListener('click', () => {
    if (carriedItemId) {
        ...
        let targetX = player.position.x + placeDir.x * 2.5; let targetZ = player.position.z + placeDir.z * 2.5;
        socket.emit('placeItem', { id: carriedItemId, x: targetX, z: targetZ, rotY: player.rotation.y });
        carriedItemId = null; actionBtn.style.display = 'none'; return;
    }
    if (isRiding) { 
        isRiding = false; mountedPig = null; socket.emit('unmountPig'); 
    ...
    if (activePickup) {
        socket.emit('pickupItem', activePickup.id);
        carriedItemId = activePickup.id; actionBtn.style.display = 'none'; return;
    }
...
function animate() {
    ...
    for (let id in localPigs) {
        ...
        npc.mesh.position.x = THREE.MathUtils.lerp(npc.mesh.position.x, npc.targetX, 0.2); npc.mesh.position.z = THREE.MathUtils.lerp(npc.mesh.position.z, npc.targetZ, 0.2);
        let rotDiff = npc.targetRotY - npc.mesh.rotation.y; rotDiff = Math.atan2(Math.sin(rotDiff), Math.cos(rotDiff)); npc.mesh.rotation.y += rotDiff * 0.2;
        if (npc.state === 'ridden') {
            if (npc.riddenBy === socket.id) { npc.mesh.position.set(player.position.x, player.position.y - 1.0, player.position.z); npc.mesh.rotation.y = player.rotation.y; npc.moveTime += 0.4; } 
            else if (remotePlayers[npc.riddenBy]) { let otherP = remotePlayers[npc.riddenBy]; npc.mesh.position.set(otherP.position.x, otherP.position.y - 1.0, otherP.position.z); npc.mesh.rotation.y = otherP.rotation.y; npc.moveTime += 0.4; }
        } else {
            ...
            let npcIntersects = raycaster.intersectObjects(solidGrounds, false); if (npcIntersects.length > 0) npcGroundY = npcIntersects[0].point.y;
            npc.velocityY += gravity; npc.mesh.position.y += npc.velocityY; if (npc.mesh.position.y <= npcGroundY) { npc.mesh.position.y = npcGroundY; npc.velocityY = 0; }
            if (npc.state === 'walking') npc.moveTime += 0.25;
        }
        if (npc.state === 'walking' || npc.state === 'ridden') { npc.legs[0].rotation.x = Math.sin(npc.moveTime) * 0.5; npc.legs[3].rotation.x = Math.sin(npc.moveTime) * 0.5; npc.legs[1].rotation.x = -Math.sin(npc.moveTime) * 0.5; npc.legs[2].rotation.x = -Math.sin(npc.moveTime) * 0.5; } 
        else { npc.legs.forEach(leg => leg.rotation.x = THREE.MathUtils.lerp(leg.rotation.x, 0, 0.1)); }
    }

    let isInsideHouse = false;
    for (let house of interactiveHouses) { if (house.box.containsPoint(player.position)) { house.roof.visible = false; isInsideHouse = true; } else { house.roof.visible = true; } }
        ...
        if (carriedItemId) {
            ...
            if (!activeBed) {
                ...
                for (let id in localItems) {
                    ...
                    let dist = player.position.distanceTo(localItems[id].group.position);
                    if (dist < minDist) { minDist = dist; activePickup = localItems[id]; }
                }
            }
            if (!activeBed && !activePickup) { 
    ...
    if (!isSleeping) {
        ...
        let intersects = raycaster.intersectObjects(solidGrounds, false); if (intersects.length > 0) currentGroundY = intersects[0].point.y;
        let targetY = isRiding ? currentGroundY + 1.0 : currentGroundY; velocityY += gravity; player.position.y += velocityY;
        ...
        const moveDir = new THREE.Vector3(); if (inputY !== 0) moveDir.addScaledVector(camForward, inputY); if (inputX !== 0) moveDir.addScaledVector(camRight, inputX); if (moveDir.lengthSq() > 1) moveDir.normalize();

        if (moveDir.lengthSq() > 0) {
            const nextPos = player.position.clone().addScaledVector(moveDir, currentSpeed); let isColliding = false;
            playerBox.min.set(nextPos.x - 0.25, player.position.y, nextPos.z - 0.25); playerBox.max.set(nextPos.x + 0.25, player.position.y + 1.5, nextPos.z + 0.25);
            ...
            if (!isColliding) {
                for (let id in localPigs) { if (localPigs[id] === mountedPig) continue; let npcBox = new THREE.Box3().setFromObject(localPigs[id].mesh); npcBox.expandByScalar(-0.1); if (playerBox.intersectsBox(npcBox)) { isColliding = true; break; } }
            }
            ...
            if (isRiding) { armL.rotation.x = 0.4; armR.rotation.x = 0.4; legL.rotation.x = -0.4; legR.rotation.x = -0.4; legL.rotation.z = -0.2; legR.rotation.z = 0.2; } 
            else { moveTime += (0.25 * (isJoyActive ? Math.max(Math.abs(joyX), Math.abs(joyY)) : 1)); armL.rotation.x = Math.sin(moveTime) * 0.8; armR.rotation.x = -Math.sin(moveTime) * 0.8; legL.rotation.x = -Math.sin(moveTime) * 0.8; legR.rotation.x = Math.sin(moveTime) * 0.8; legL.rotation.z = 0; legR.rotation.z = 0; }
        } else {
            player.rotation.x = 0;
            if (isRiding) { armL.rotation.x = 0.4; armR.rotation.x = 0.4; legL.rotation.x = -0.4; legR.rotation.x = -0.4; } 
            else { armL.rotation.x = THREE.MathUtils.lerp(armL.rotation.x, 0, 0.1); armR.rotation.x = THREE.MathUtils.lerp(armR.rotation.x, 0, 0.1); legL.rotation.x = THREE.MathUtils.lerp(legL.rotation.x, 0, 0.1); legR.rotation.x = THREE.MathUtils.lerp(legR.rotation.x, 0, 0.1); legL.rotation.z = 0; legR.rotation.z = 0; }
        }
    } 

    for (let id in remotePlayers) {
        const p = remotePlayers[id]; const anim = p.userData; const isMoving = p.position.distanceTo(anim.lastPos) > 0.01; 
        if (isMoving) { anim.moveTime += 0.2; const mt = anim.moveTime; anim.armL.rotation.x = Math.sin(mt) * 0.8; anim.armR.rotation.x = -Math.sin(mt) * 0.8; anim.legL.rotation.x = -Math.sin(mt) * 0.8; anim.legR.rotation.x = Math.sin(mt) * 0.8; } 
        else { anim.armL.rotation.x = THREE.MathUtils.lerp(anim.armL.rotation.x, 0, 0.1); anim.armR.rotation.x = THREE.MathUtils.lerp(anim.armR.rotation.x, 0, 0.1); anim.legL.rotation.x = THREE.MathUtils.lerp(anim.legL.rotation.x, 0, 0.1); anim.legR.rotation.x = THREE.MathUtils.lerp(anim.legR.rotation.x, 0, 0.1); }
