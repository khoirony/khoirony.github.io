...
const fishingRod = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 2.5), new THREE.MeshStandardMaterial({ color: 0x5c4033 })); fishingRod.position.set(0, -0.7, 1); fishingRod.visible = false; armR.add(fishingRod);
...
const jumpBtn = document.getElementById('jump-btn'); const triggerJump = (e) => { e.preventDefault(); if (!isJumping && !isSleeping && !isFishing && !isDrowning && !isShaking) { velocityY = 0.3; isJumping = true; } }; jumpBtn.addEventListener('touchstart', triggerJump, { passive: false }); jumpBtn.addEventListener('mousedown', triggerJump);

initNetworking(scene, player);

let localPigs = {}; let localItems = {}; let carriedItemId = null; 
let localStats = { coins: 0, fish: 0, apples: 0 }; let isFishing = false; let isDrowning = false; let isShaking = false;
...
actionBtn.addEventListener('click', () => {
    if (actionMenu.style.display === 'block') {
        ...
        currentAvailableActions.forEach((act, index) => {
            ...
            btn.innerHTML = act.label; // Label HANYA berisi ikon emoji sekarang
            ...
            // RADIUS DAN SUDUT PENYEBARAN IKON
            let angle;
            if (total === 1) {
                angle = Math.PI / 4; 
            } else {
                angle = (index / (total - 1)) * (Math.PI / 2);
            }
...
function animate() {
    ...
    if (!isSleeping && !isFishing && !isDrowning && !isShaking) {
        if (carriedItemId) { 
            ...
            currentAvailableActions.push({
                label: '🚶', // Ikon Dismount
                execute: () => {
                    isRiding = false; mountedPig = null; socket.emit('unmountPig'); player.position.x += 1; player.position.y += 0.5;
                }
            });
        } else {
            ...
            if (nearWater) { 
                currentAvailableActions.push({
                    ...
                    execute: () => {
                        ...
                        setTimeout(() => { isFishing = false; fishingRod.visible = false; actionBtn.style.pointerEvents = 'auto'; actionBtn.innerHTML = '⚙️'; if (Math.random() > 0.4) { socket.emit('catchFish'); } }, 3000);
                    }
                }); 
            }
            for (let bed of beds) { 
                if (bed.triggerBox.containsPoint(player.position)) { 
                    currentAvailableActions.push({
                        label: '💤',
                        execute: () => {
                            isSleeping = true; previousPlayerPos.copy(player.position); previousPlayerRot.copy(player.rotation); player.position.copy(bed.sleepPosition); player.rotation.set(-Math.PI / 2, 0, bed.sleepRotY); velocityY = 0; fadeOverlay.style.opacity = 1; 
            ...
            for (let id in localPigs) { 
                if ((localPigs[id].state === 'idle' || localPigs[id].state === 'walking') && player.position.distanceTo(localPigs[id].mesh.position) < 3.0) { 
                    currentAvailableActions.push({
                        label: '🐎',
                        execute: () => { isRiding = true; mountedPig = localPigs[id]; socket.emit('ridePig', localPigs[id].id); }
                    });
                    break; 
                } 
            }
            ...
            for (let id in localItems) { 
                let itm = localItems[id];
                if (itm.carriedBy) continue; 
                ...
                if (itm.type === 'tree' && dist < 4.0) {
                    currentAvailableActions.push({
                        label: '🌳',
                        execute: () => {
                            isShaking = true; actionBtn.innerHTML = '⏳'; actionBtn.style.pointerEvents = 'none';
                ...
                if (itm.type === 'torch' && !isDay && !itm.isLit && dist < 3.0) {
                    currentAvailableActions.push({
                        label: '🔥',
                        execute: () => socket.emit('lightTorch', itm.id)
                    });
                }
            }
            ...
            if (closestPickup) {
                currentAvailableActions.push({
                    label: '🖐️', // Ikon Pickup
                    execute: () => { socket.emit('pickupItem', closestPickup.id); carriedItemId = closestPickup.id; }
                });
            }
        }
    }
    ...
    if (currentAvailableActions.length > 0 && !isSleeping && !isFishing && !isDrowning && !isShaking) {
        actionBtn.style.display = 'flex'; // Pakai flex agar logo di tengah
