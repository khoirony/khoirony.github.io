...
function createLimb(w, h, d, color, yPivot) { 
    ...
    mesh.castShadow = true; mesh.position.y = -h / 2; group.add(mesh); group.position.y = yPivot; 
    return group; 
}

const body = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.75, 0.25), new THREE.MeshStandardMaterial({ color: 0x00aaff })); 
body.position.y = 1.125; body.castShadow = true; player.add(body);
...
socket.on('initItems', (serverItems) => {
    for (let id in serverItems) { 
        ...
        if (d.type === 'tree') objMesh = createTreeMesh(); else if (d.type === 'torch') objMesh = createTorchMesh(); else if (d.type === 'mading') objMesh = createMadingMesh(d.title, d.lines); 
        let groupToSpawn = (d.type === 'torch') ? objMesh.mesh : objMesh; scene.add(groupToSpawn); 
        ...
        if (d.type === 'torch' && localItems[id].torchData) { localItems[id].torchData.fire.visible = d.isLit; localItems[id].torchData.light.intensity = d.isLit ? 150 : 0; }
    }
});
...
socket.on('updateItem', (data) => { 
    ...
    if (item) { 
        item.carriedBy = data.data.carriedBy; item.targetX = data.data.x; item.targetZ = data.data.z; item.targetRotY = data.data.rotY; item.isLit = data.data.isLit;
        if (item.type === 'torch') { item.torchData.fire.visible = item.isLit; item.torchData.light.intensity = item.isLit ? 150 : 0; }
    } 
});
...
socket.on('timeUpdate', (data) => {
    ...
    if (newIsDay !== isDay && scene) {
        isDay = newIsDay;
        if(isDay) { scene.background = new THREE.Color(0x87CEEB); scene.fog.color.setHex(0x87CEEB); dirLight.intensity = 1.0; ambientLight.intensity = 0.5; document.body.style.background = '#87CEEB'; } 
        else { scene.background = new THREE.Color(0x05051a); scene.fog.color.setHex(0x05051a); dirLight.intensity = 0.1; ambientLight.intensity = 0.1; document.body.style.background = '#05051a'; }
    }
...
actionBtn.addEventListener('click', () => {
    if (actionMenu.style.display === 'block') {
        actionMenu.style.display = 'none'; actionBtn.innerHTML = '⚙️'; actionBtn.style.background = 'rgba(255, 215, 0, 0.8)'; actionBtn.style.color = 'black';
    } else {
        actionMenu.innerHTML = ''; let total = currentAvailableActions.length;
        currentAvailableActions.forEach((act, index) => {
            let btn = document.createElement('button'); btn.innerHTML = act.label; 
            btn.onclick = () => { actionMenu.style.display = 'none'; actionBtn.innerHTML = '⚙️'; actionBtn.style.background = 'rgba(255, 215, 0, 0.8)'; actionBtn.style.color = 'black'; act.execute(); };
            let angle = total === 1 ? Math.PI / 4 : (index / (total - 1)) * (Math.PI / 2);
            let radius = window.innerWidth < 800 ? 75 : 90; 
            btn.style.right = `${Math.cos(angle) * radius}px`; btn.style.bottom = `${Math.sin(angle) * radius}px`;
            btn.style.animation = `popInRadial 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards`; btn.style.animationDelay = `${index * 0.05}s`;
            actionMenu.appendChild(btn);
        });
        actionMenu.style.display = 'block'; actionBtn.innerHTML = '✖'; actionBtn.style.background = 'rgba(255, 68, 68, 0.9)'; actionBtn.style.color = 'white';
    }
});
...
function animate() {
    ...
    if (!isSleeping && !isFishing && !isDrowning && !isShaking) {
        ...
        // NORMAL STATE
            ...
            for (let bed of beds) { if (bed.triggerBox.containsPoint(player.position)) { currentAvailableActions.push({ label: '💤', execute: () => { isSleeping = true; previousPlayerPos.copy(player.position); previousPlayerRot.copy(player.rotation); player.position.copy(bed.sleepPosition); player.rotation.set(-Math.PI / 2, 0, bed.sleepRotY); velocityY = 0; fadeOverlay.style.opacity = 1; setTimeout(() => { socket.emit('requestTimeSkip'); setTimeout(() => { fadeOverlay.style.opacity = 0; player.position.copy(previousPlayerPos); player.rotation.copy(previousPlayerRot); isSleeping = false; }, 1500); }, 1500); } }); break; } }
            for (let id in localPigs) { if ((localPigs[id].state === 'idle' || localPigs[id].state === 'walking') && player.position.distanceTo(localPigs[id].mesh.position) < 3.0) { currentAvailableActions.push({ label: '🐎', execute: () => { isRiding = true; mountedPig = localPigs[id]; socket.emit('ridePig', localPigs[id].id); } }); break; } }
            ...
            for (let id in localItems) { 
                ...
                if (itm.type === 'tree' && dist < 4.0) { currentAvailableActions.push({ label: '🌳', execute: () => { isShaking = true; actionBtn.innerHTML = '⏳'; actionBtn.style.pointerEvents = 'none'; setTimeout(() => { isShaking = false; actionBtn.style.pointerEvents = 'auto'; actionBtn.innerHTML = '⚙️'; socket.emit('shakeTree'); }, 2000); } }); }
                if (itm.type === 'torch' && !isDay && !itm.isLit && dist < 3.0) { currentAvailableActions.push({ label: '🔥', execute: () => socket.emit('lightTorch', itm.id) }); }
            }
            if (closestPickup) { currentAvailableActions.push({ label: '🖐️', execute: () => { socket.emit('pickupItem', closestPickup.id); carriedItemId = closestPickup.id; } }); }
        }
    }
